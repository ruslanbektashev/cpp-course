...
- `print_ip.h` — шаблонная функция `print_ip()` с вариантами через SFINAE:
  - целочисленные типы (побайтово, старший байт первым)
  - `std::string` (как есть)
  - `std::vector` / `std::list` (элементы через `.`)
- `matrix.h` — заголовочный файл с реализацией бесконечной матрицы `Matrix<T, DefaultValue>`,
  которая хранит только отличающиеся от `DefaultValue` значения.
...
Если уже есть папка build, то для пересборки достаточно:

cmake --build build --clean-first
Если хочешь «с нуля» (пересоздать конфигурацию):

Remove-Item -Recurse -Force build
cmake -S . -B build
cmake --build build
Запуск (как у тебя под MS
